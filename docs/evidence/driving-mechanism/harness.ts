...
import { ResponsesApiError, ResponsesModel, type DrivingMechanism } from '../../../src/agent/responses-model'
...
try {
  for (const mechanism of mechanisms) {
    for (const imagePath of positionals.length > 0 ? positionals : DEFAULT_IMAGES) {
      records.push(await measure(mechanism, imagePath))
    }
  }
} finally {
  await browser.close()
...
async function measure(mechanism: DrivingMechanism, imagePath: string): Promise<RunRecord> {
  const image = basename(imagePath)
  const runId = `${mechanism}-${image}`
  ...
  const responses = dryRun
    ...
    : new ResponsesModel({
        ...
        stepCap,
        mechanism,
        codeRunner: pageCodeRunner(page)
      })
  ...
  const record: RunRecord = {
    ...
    threeEditsVisible,
    completed: snapshot.status === 'complete' && threeEditsVisible
  }
  ...
  const rows = (['computer', 'code'] as const).flatMap((mechanism) => {
    ...
    return [
      `| ${mechanism} | ${mine.filter((run) => run.completed).length}/${mine.length} | ${mean((run) => run.steps).toFixed(1)} | ` +
        ...
        `${total((run) => run.tokensOut)} | $${total((run) => run.costUsd).toFixed(2)} | ${total((run) => run.silentSteps)} |`
    ]
  })
  return [
    `# Spike A0 results${dryRun ? ' (dry run: scripted model, measures nothing)' : ''}`,
    '',
    '| Mechanism | Completed | Mean steps | Mean wall clock | Tokens in | Tokens out | Cost | Silent steps |',
    '| --------- | --------- | ---------- | --------------- | --------- | ---------- | ---- | ------------ |',
    ...rows,
    '',
    ...runs.map(
