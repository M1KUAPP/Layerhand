...
const fixtureEvidence = await persistInputFixture(OUTPUT_DIRECTORY, jpeg)
...
try {
  ...
  const roundTripMs = performance.now() - uploadStartedAt
  ...
  const result = {
    ...
    fixturePath: recordOutputPath('input.jpg'),
    ...
    psdBytes: psd.length,
    psdPath: recordOutputPath('photopea-round-trip.psd'),
    readyMs: Math.round(readyMs),
    roundTripMs: Math.round(roundTripMs),
    screenshotPath: recordOutputPath('photopea-final.png'),
