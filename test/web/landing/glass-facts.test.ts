...
test('the three glass facts share one equal 40px icon column with centered glyphs', async () => {
  ...
  expect(css).toMatch(/\.glass__fact-icon\s*\{[^}]*align-self:\s*start/)
  expect(css).toMatch(/\.glass__fact-icon\s*\{[^}]*grid-row:\s*1\s*\/\s*3/)
  expect(css).toMatch(/\.glass__fact-title,\s*\n\s*\.glass__fact-body\s*\{[^}]*grid-column:\s*2/)
